...
const ProjectDiscoveryWizard = ({ onClose }: { onClose: () => void }) => {
  ...
  const renderStep = () => {
    ...
    if (step === 3) return <div className="space-y-5"><div><p className="mb-3 text-sm font-medium text-gray-200">{t('fields.pageRange')} <span className="text-red-groundlight">*</span></p><div className="grid gap-2 sm:grid-cols-3">{['1–3', '4–7', '8–15', '16+', "I'm not sure"].map((item) => <Choice key={item} fieldName="pageRange" invalid={invalidField === 'pageRange'} label={item} selected={value(data, 'pageRange') === item} onClick={() => setField('pageRange', item)} />)}</div></div><TextField label={t('fields.expectedPages')} name="pages" value={value(data, 'pages')} onChange={setField} placeholder={t('placeholders.pages')} /><div><p className="mb-3 text-sm font-medium text-gray-200">{t('fields.contentStatus')}</p><div className="grid gap-2 sm:grid-cols-2">{['I have everything', 'I have some content', 'I need help creating content', "I don't know yet"].map((item) => <Choice key={item} label={item} selected={value(data, 'contentStatus') === item} onClick={() => setField('contentStatus', item)} />)}</div></div></div>;
    if (step === 4) return <div className="space-y-5"><div><p className="mb-3 text-sm font-medium text-gray-200">{t('fields.languageCount')} <span className="text-red-groundlight">*</span></p><div className="grid gap-2 sm:grid-cols-2">{['oneLanguage', 'twoLanguages', 'threeLanguages', 'moreLanguages'].map((key) => <Choice key={key} fieldName="languageCount" invalid={invalidField === 'languageCount'} label={t(`options.${key}`)} selected={value(data, 'languageCount') === key} onClick={() => setField('languageCount', key)} />)}</div></div><div><p className="mb-3 text-sm font-medium text-gray-200">{t('fields.languages')} <span className="text-red-groundlight">*</span></p><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{['English', 'Arabic', 'French', 'German', 'Spanish', 'Turkish', 'Other'].map((item) => <Choice key={item} fieldName="languages" invalid={invalidField === 'languages'} label={item} selected={listValue(data, 'languages').includes(item)} onClick={() => { setData(toggleValue(data, 'languages', item)); setError(''); setInvalidField(null); }} />)}</div></div><div><p className="mb-3 text-sm font-medium text-gray-200">{t('fields.translations')}</p><div className="grid gap-2 sm:grid-cols-3">{['I provide translations', 'I need help', 'Add translations later'].map((item) => <Choice key={item} label={item} selected={value(data, 'translations') === item} onClick={() => setField('translations', item)} />)}</div></div></div>;
    if (step === 5) return <div className="space-y-5"><div><p className="mb-3 text-sm font-medium text-gray-200">{t('fields.design')} <span className="text-red-groundlight">*</span></p><div className="grid gap-2 sm:grid-cols-2">{['I already have a design', 'I have references', 'I have an idea', 'You design it'].map((item) => <Choice key={item} fieldName="design" invalid={invalidField === 'design'} label={item} selected={value(data, 'design') === item} onClick={() => setField('design', item)} />)}</div></div><TextField label={t('fields.references')} name="references" value={value(data, 'references')} onChange={setField} placeholder={t('placeholders.references')} /><div className="grid gap-2 sm:grid-cols-2">{['Complete brand identity', 'Logo only', 'Partial branding', 'No branding yet'].map((item) => <Choice key={item} label={item} selected={value(data, 'brand') === item} onClick={() => setField('brand', item)} />)}</div></div>;
